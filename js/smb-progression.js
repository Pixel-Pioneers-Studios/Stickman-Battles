...
function advanceMotivation() {
    ...
    motivationStage++;
    try { localStorage.setItem('smb_motivation_stage', String(motivationStage)); } catch (e) { /* ignore */ }
...
function _saveShipState() {
    try {
        localStorage.setItem('smb_ship', JSON.stringify(SHIP));
...
function _loadProgressionState() {
    try {
        ...
        if (ship) {
            const parsed = JSON.parse(ship);
            Object.assign(SHIP, parsed);
        }
    } catch (e) { /* ignore */ }
