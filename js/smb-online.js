...
const LobbyManager = (() => {
  ...
  function createLobby(opts) {
    ...
    if (window.NetworkManager && typeof NetworkManager.connect === 'function') {
      NetworkManager.connect(id, _current.maxPlayers).catch(function(e) {
  ...
  function joinLobby(id) {
    ...
    _current = {
      ...
      players:    meta ? meta.players.slice() : [{ slot: 0, name: isPub ? 'Public Server' : 'Host' }],
      maxPlayers: meta ? meta.maxPlayers : (isPub ? 10 : 2),
      isPrivate:  meta ? !!meta.isPrivate : false,
      persistent: isPub,
    };
    _current.players.push({ slot: _current.players.length, name: _playerName() });
