...
// ============================================================
// BOSS FINISHER 2: REALITY BREAK
// Theme: glitch / creator power — player distorts and launches
// ============================================================
const FIN_REALITY_BREAK = {
  ...
  setup(att, tgt, data) {
    ...
    data.distortScale = 1;
    data.launchX = data.tx0 + data.dir * 320;
    ...
    data.timeline = _makeTimeline([
      ...
      // Boss walks toward player slowly
      { frame: 30, fn() { CinCam.slowMo(0.25); } },
      // Boss touches player — distortion
      { frame: 75, fn() {
          data.glitchIntensity = 2.5;
          data.distortScale = 1;
          CinCam.shake(18);
      ...
      { frame: 90, fn() {
          ...
          data.glitchIntensity = 0;
          data.distortScale = 1;
          CinCam.zoomTo(0.9); // zoom out to follow the launch
      }},
      // Camera follows launched player
      { frame: 95, fn() { CinCam.focusOn(tgt); } },
      // Camera crash-follow
      { frame: 130, fn() {
          CinCam.shake(22);
          spawnParticles(data.launchX+tgt.w/2, data.launchY+tgt.h, '#00ffcc', 35);
          spawnParticles(data.launchX+tgt.w/2, data.launchY+tgt.h, '#ffffff', 15);
      }},
      { frame: 150, fn() {
          CinCam.zoomTo(1.0);
          CinCam.restore();
      }},
      { frame: 165, fn() { data.bars = 0; } },
    ]);
  },
  ...
  update(att, tgt, timer, data) {
    ...
    if (timer > 165)    data.bars = Math.max(0, (180-timer)/15);

    // White flash decay
    if (data.whiteAlpha > 0) data.whiteAlpha = Math.max(0, data.whiteAlpha - 0.09);
    ...
    // Phase 2 (30-80): boss walks toward player
    } else if (timer <= 80) {
      const t2 = (timer-30)/50;
      const targetBossX = data.tx0 - data.dir * 22;
      att.x = _finLerp(data.ax0, targetBossX, _finEaseInOut(t2));
      att.y = data.ay0;
      tgt.x = data.tx0; tgt.y = data.ty0;

    // Phase 3 (80-90): distort / white flash
    } else if (timer <= 90) {
      data.distortScale = 1 + Math.sin((timer-80)*0.6) * 0.3;
      tgt.x = data.tx0 + (Math.random()-0.5)*8*data.glitchIntensity;
      tgt.y = data.ty0 + (Math.random()-0.5)*6*data.glitchIntensity;

    // Phase 4 (90-130): violent launch across arena
    } else if (timer <= 130) {
      const t2 = (timer-90)/40;
      tgt.x = _finLerp(data.tx0, data.launchX, _finEaseIn(t2));
      tgt.y = _finLerp(data.ty0, data.launchY, t2 < 0.5 ? _finEaseOut(t2*2)*(-0.4) : _finEaseIn((t2-0.5)*2)*0.6 - 0.4 + 1);

    // Phase 5 (130+): landed
    } else {
      tgt.x = data.launchX; tgt.y = data.launchY;
      att.x = data.tx0 - data.dir * 22; att.y = data.ay0;
    }
  ...
  draw(ctx, att, tgt, t, timer, data) {
    ...
    if (data.glitchIntensity > 0) {
      ctx.save(); ctx.setTransform(1,0,0,1,0,0);
      const gi = Math.min(1, data.glitchIntensity / 3);
      for (let i = 0; i < 6; i++) {
        const gy2 = Math.random() * canvas.height;
        const gh  = 2 + Math.random() * 8;
        const gx2 = (Math.random()-0.5) * canvas.width * 0.12 * gi;
        ctx.fillStyle = `rgba(0,255,200,${gi * 0.18})`;
        ctx.fillRect(gx2, gy2, canvas.width, gh);
      }
      ctx.restore();
    }

    // Boss energy aura (cyan/teal)
    if (timer > 30 && timer < 95) {
      ...
      grd.addColorStop(0,`rgba(0,220,160,${ba*0.28})`);
      grd.addColorStop(1,'rgba(0,220,160,0)');
      ctx.fillStyle = grd; ctx.beginPath();
      ctx.arc(att.cx(),att.cy(),70,0,Math.PI*2); ctx.fill();
      ctx.restore();
    }

    // Reality distortion rings around target
    if (timer > 60 && timer < 90) {
      const ra = Math.min(1,(timer-60)/15);
      ctx.save(); ctx.setTransform(scX,0,0,scY,ox,oy);
      for (let i = 0; i < 3; i++) {
        const rr = 20 + i*18 + Math.sin(timer*0.3+i)*10;
        ctx.strokeStyle = `rgba(0,255,200,${ra*(0.7-i*0.18)})`;
        ctx.lineWidth = 2 + i;
    ...
    // Launch trail (motion blur lines)
    if (timer > 90 && timer < 130) {
      ctx.save(); ctx.setTransform(scX,0,0,scY,ox,oy);
      const trailAlpha = 0.45;
      ctx.strokeStyle = `rgba(0,255,200,${trailAlpha})`;
      ctx.lineWidth = 3; ctx.shadowColor = '#00ffcc'; ctx.shadowBlur = 18;
      ctx.beginPath(); ctx.moveTo(tgt.cx(), tgt.cy());
      ctx.lineTo(tgt.cx() - data.dir*40, tgt.cy() + 15);
      ctx.stroke();
      ctx.restore();
    }
    ...
    if (data.whiteAlpha > 0) {
      _finFlash(ctx, data.whiteAlpha, 200, 255, 240);
    }
...
const CLASS_FINISHERS = {
  ...
  megaknight: _wfDef('FINAL JUDGMENT','rgba(220,180,40,1)',128,
    ...
    (att,tgt,timer,data)=>{ _tickTimeline(data.tl,timer); _wfTick(timer,128,data);
      ...
      else if(timer<44){ const p=(timer-30)/14; att.x=data.ax0+data.dir*_finEaseOut(p)*60; att.y=data.ay0-90+_finEaseIn(p)*(90+145); tgt.x=data.tx0;tgt.y=data.ty0; data.auraAlpha=0.88; data.auraR=54; }
      else{ const p=Math.min(1,(timer-44)/58); tgt.x=data.tx0; tgt.y=data.ty0+_finEaseOut(p)*68; att.x=data.ax0+data.dir*60;att.y=data.ay0+145; data.auraAlpha=Math.max(0,data.auraAlpha-0.04); }
      att.vx=0;att.vy=0;tgt.vx=0;tgt.vy=0; },
    (ctx,att,tgt,t,timer,data)=>{
      const {scX,scY,ox,oy}=_finGameTransform();
      _finBars(ctx,data.bars); _finVignette(ctx,0.46);
...
function _pickFinisher(attacker) {
  ...
  const haC  = cKey && cKey !== 'none' && !!(CLASS_FINISHERS[cKey]);
